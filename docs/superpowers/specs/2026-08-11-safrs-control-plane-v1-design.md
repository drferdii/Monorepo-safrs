...
- Review completion is recorded in `.safrs/reviews/verification-integrity.json`. The classifier accepts it only when its approved verdict, reviewer identity, UTC timestamp, strict schema, and SHA-256 fingerprint match the exact current change set. Missing, malformed, or stale evidence fails closed.
