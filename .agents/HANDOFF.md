...
- **Interactive & Visual features:**
  - Hero 2-column layout: text shifted to left column with high-contrast Mona Sans typography & profile badge; 3 interactive chips (*Arsitektur Kecerdasan Buatan*, *Desain Situs Web*, *Otomasi Cerdas*) cleanly spaced across two rows with zero overlap; right column equipped with realistic native macOS Terminal UI (`novia@Novia-MBP ~/sentra %`).
