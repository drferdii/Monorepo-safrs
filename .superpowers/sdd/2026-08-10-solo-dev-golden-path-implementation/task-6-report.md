...
- `scripts/setup.mjs`: creates `.env` with exclusive creation only, preserves concurrent `EEXIST`, rejects dangling symlinks, validates the canonical disposable local database, then runs install, local PostgreSQL, generate, committed migrations, deterministic seed, and doctor.
...
- `tests/repository/operator-commands.test.mjs`: proves setup against fixtures, exclusive-create races/symlinks, canonical-root override over a malicious app-local URL, strict Turbo declarations, development preflight/start ordering, and pnpm's reserved bare doctor command.
...
- `node --test tools/doctor/test/*.test.mjs scripts/lib/*.test.mjs tests/repository/operator-commands.test.mjs` — 21 pass, 0 fail.
